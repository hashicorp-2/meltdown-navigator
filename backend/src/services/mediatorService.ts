...
function buildPersonalizedSystemPrompt(profile: {
  ...
  return (
    COMMUNICATION_MEDIATOR_SYSTEM_PROMPT +
    toneGuidance +
    doPhrases +
    avoidPhrases +
...
function buildUserPrompt(input: CommunicationMediatorRequestDTO): string {
  const {
    rawMessage,
    senderContext,
    recipientContext,
  ...
  async mediate(
    ...
    if (request.profileId) {
      try {
        ...
        console.warn(
          '[MediatorService] Failed to load profile, continuing without personalization',
          {
            profileId: request.profileId,
    ...
    } satisfies CommunicationMediatorResponseDTO;
    ...
    try {
      ...
      if (error instanceof ZodError) {
        throw new Error(
          `Mediator response validation failed: ${error.message}`,
        );
      }
      throw error;
    }
